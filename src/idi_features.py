"""
idi_features.py  [FIXED v2]
---------------
IDI Feature Extraction — npj Multinational Validation

FIXES APPLIED:
  ISSUE-9 : Changed np.std(deltas) → np.std(deltas, ddof=1) to match
            MIMIC-IV repo (sample std, not population std). Affects
            idi_std_interevent_min, idi_cv_interevent, and idi_burstiness.
  ISSUE-10: window_hours parameter now ACTUALLY FILTERS events to the
            first N hours from admission (intime_series must be supplied).
            Added clear docstring warning when pre-filtering is the
            caller's responsibility (backward-compatible default).
  MINOR-14: Removed unused `from scipy.stats import variation` import.
  MINOR   : Precision improvement — inter-event intervals now computed
            in seconds then converted to float minutes (avoids integer
            truncation from timedelta64[m]).
...
def compute_idi_features(
    ...
    events_df : pd.DataFrame
        ...
        IMPORTANT: If intime_series is None, the caller MUST pre-filter
        events to the desired analysis window before calling this function.
        Passing unfiltered events will produce incorrect IDI values.
    stay_id_col : str
        Column name for ICU stay identifier.
    time_col : str
        Column name for event timestamp (datetime or parseable string).
    window_hours : int
        Analysis window in hours (default 24).  Used to:
          (a) filter events if intime_series is provided, AND
          (b) compute idi_events_per_hour = n / window_hours.
    intime_series : pd.Series or None
        If provided, must be indexed by stay_id_col values and contain
        the ICU admission datetime for each stay.  Events outside
        [0, window_hours) from intime will be dropped.
        If None, no filtering is applied — caller is responsible.

    Returns
    -------
    pd.DataFrame
        One row per stay_id with 9 IDI features.
        Stays with < 2 valid intervals receive NaN for all features.
    """
    ...
    for stay_id, grp in events_df.groupby(stay_id_col):
        # ── FIX ISSUE-10: optional 24-hour window filter ──────────────────
        ...
        # FIX MINOR: compute deltas in seconds → convert to float minutes
        # (avoids integer truncation of timedelta64[m])
        deltas_sec = np.diff(times).astype("timedelta64[s]").astype(float)
        deltas     = deltas_sec / 60.0            # minutes as float
        ...
        # ── FIX ISSUE-9: use ddof=1 (sample std) to match MIMIC-IV repo ──
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Extract IDI features from a pre-filtered events CSV.\n"
            "IMPORTANT: --events file must already be filtered to the\n"
            "first 24 hours from ICU admission per stay.  If your events\n"
            "file is unfiltered, provide --cohort to enable auto-filtering."
        )
    )
    parser.add_argument("--cohort",  required=False,
                        help="Path to cohort CSV with stay_id and intime columns "
                             "(enables auto 24-h window filter)")
    ...
    if args.cohort:
        ...
        print("WARNING: no --cohort provided. Events must be pre-filtered "
