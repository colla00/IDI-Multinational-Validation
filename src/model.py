...
model.py
--------
Model training for the multinational IDI validation study.

Trains baseline and IDI-enhanced logistic regression models on the MIMIC-IV
derivation cohort. Supports both temporal (2008-2018 train / 2019 test) and
...
Pipeline: median imputation -> z-score scaling -> logistic regression (C=1.0).
Median imputation uses training-set medians to prevent test-set leakage, and
handles NaN IDI values that can arise from stays with sparse chartevent data.

Leakage filter: IDI features with |Pearson r| > 0.30 with ICU LOS are removed
before training. Non-survivors have longer ICU stays (median 71.3 h vs 53.1 h),
so features correlated with LOS are post-outcome proxies rather than predictors.
...
# Ensure src/ is on the path when called with `python src/model.py`
...
def apply_leakage_filter(train_df: pd.DataFrame,
    ...
    Filter is computed on training data only.
    """
    kept, dropped = [], []
...
def build_pipeline() -> Pipeline:
    """Median imputation -> z-score scaling -> logistic regression."""
    return Pipeline([
...
def train_and_evaluate(data_path: str, split: str = "temporal",
    ...
    if save_model:
        ...
        print("Model saved      -> results/model_idi.pkl")
